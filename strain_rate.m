...
%
% Reads csv file generated by Paraview of the particles.
%
% Inputs: 'filename', number of final timestep for particles, time 
%   factor (how many timesteps in a second)
%
% Outputs: Plot of strain rate at center of mass over time.
%
...
finaltime=7900;
steps_per_second=100;   % Number of timesteps per second.
box_size=5;             % Radius of box.
strain_rates=[];
time=[];
...
for timestep=0:finaltime
    ...
    time=cat(1,time,timestep/steps_per_second);
    ...
    strain_rates=cat(1,strain_rates,sqrt((dv_dx)^2+(du_dy)^2));
end

plot(time,strain_rates)
title('Strain Rate Over Time at Center of Mass (0.58)')
xlabel('Time (s)')
ylabel('Strain Rate')
