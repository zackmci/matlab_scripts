...
%finaltime=50;
time_factor=0.01;
angle_bins=36;      % Number of bins for the circle
...
for timestep=1:finaltime,
    ...
    orientations=mod(atan2(new_data(:,7)-new_data(:,5), ...
        new_data(:,6)-new_data(:,4)),2*pi);
    
    orientations=orientations+adjusted_angle;
    
    % To make sure there are no negative angles, this portion checks for
    % negative numbers and adds 2*pi to them.
    final_row=size(orientations,1);
    for array_position=1:final_row,
        if orientations(array_position)<0;
            orientations(array_position)=orientations(array_position)+2*pi;
        else
            orientations(array_position)=orientations(array_position);
        end
    end
